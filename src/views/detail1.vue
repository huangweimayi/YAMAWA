...
<!--      <el-row>
      ...
      <table>
        <tr class="grey" :class="{'bt_none':index!=0,'mb_20':index == down.length-1}" v-for="(item,index) in down">
          <td class="t_a left">{{item.name}}</td>
          <td class="t_a">{{item.value}}</td>
        </tr>
      </table>
...
<script>
  ...
  export default {
    ...
    data(){
      return{
        ...
        up:[
          ...
          {name:"底孔形状通孔",value:'',code:'through_hole'},
          {name:"底孔形状盲孔",value:'',code:'blind_hole'},
          {name:"全长",value:'',code:'length'},
          {name:"螺纹部长",value:'',code:'part_length'},
          {name:"颈下长(ℓn)",value:'',code:'nect_length'},
          {name:"柄长(ℓ2)",value:'',code:'handle_length'},
          {name:"柄部径(Ds)",value:'',code:'handle_diameter'},
          {name:"四角部长(ℓ2)",value:'',code:'part_num'},
          {name:"四角部宽(k)",value:'',code:'four_wide'},
          {name:"吃入部",value:'',code:'chamfer'},
          {name:"螺丝攻的种类",value:'',code:'handle_type'},
          {name:"精度等级",value:'',code:'precision_level'},
          {name:"钩数",value:'',code:'ditch_num'},
          {name:"材质",value:'',code:'material_quality'},
          {name:"外径的基准尺寸",value:'',code:'outer_reference_size'},
          {name:"到基准径的长(ℓg)",value:'',code:'to_datum_diameter'},
          {name:"表面处理",value:'',code:'surface_treatment'},
          {name:"进给(同步进给专用)",value:'',code:'Feed'},
          {name:"咬合率",value:'',code:'occlusal_rate'},
          {name:"建议底孔径",value:'',code:'proposal_bottom_diameter'},
          {name:"构型",value:'',code:'gouXing'},
        ],
        down:[
    ...
    methods:{
      getDetail(){
        let _t = this;
        api.luowenDetail({id:_t.id}).then(res => {
          ...
          for(let i in res.data){
            for(let k = 0;k<_t.up.length;k++){
              ...
              // "http://211.149.169.201/old_img/img/SP_001.jpg"
            }
            if(i == 'configuration'){
              let arr = res.data[i].split('/');
              _t.up[_t.up.length-1].value = arr[arr.length-1].split('.')[0]
            }
            for(let k = 0;k<_t.down.length;k++){
              if(i == _t.down[k].code){
                _t.down[k].value = res.data[i]
              }
            }
          }
        })
