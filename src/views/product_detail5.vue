<template>
  <div class="detail">
    ...
    <el-form class="d_form">
      <el-select v-model="search.purpose" @change="getList" @focus="getZhuan('purpose')" placeholder="用途">
        <el-option
      ...
      <el-select v-model="search.brick_diameter" placeholder="钻头径·孔径（Dc）" @change="getList"  @focus="getZhuan('brick_diameter')">
        <el-option
    ...
    <el-table
      ...
      style="width: 100%">
      <el-table-column prop="product_code" label="产品代号" width="100"></el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      tableClick(row, event, column) {
        this.$router.push({path: '/detail5',query:{id:row.id}})
      },
      getZhuan(type) {
        let _t = this;
        let data = {
          search: type,
          'data[purpose]': _t.search.purpose,//用途
          'data[brick_diameter]': _t.search.brick_diameter,//钻头径·孔径（Dc）
          'data[angle]': _t.search.angle,//角度
          'data[product_code]': _t.search.product_code,//产品代号
        };
        api.kongmian(data).then(res => {
          let getArr = [];
          switch (type) {
            case 'purpose':
              getArr.push({name: '用途', value: ''});
              for (let i = 0; i < res.data.length; i++) {
                if (res.data[i].purpose != '-') {
                  getArr.push({name: res.data[i].purpose, value: res.data[i].purpose})
                }
              }
              _t.options1 = getArr;
              break;
            case 'brick_diameter':
              getArr.push({name: '钻头径·孔径（Dc）', value: ''});
              for (let i = 0; i < res.data.length; i++) {
                res.data[i].brick_diameter != '-' ? getArr.push({name: res.data[i].brick_diameter, value: res.data[i].brick_diameter}):''
              }
              _t.options2 = getArr;
              break;
            case 'angle':
              ...
              for (let i = 0; i < res.data.length; i++) {
                res.data[i].angle != '-' ? getArr.push({name: res.data[i].angle, value: res.data[i].angle}):''
              }
              _t.options3 = getArr;
              break;
            case 'product_code':
